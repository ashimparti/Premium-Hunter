...
def score(d):
    """Strict scoring system. Hard filters auto-disqualify. Score out of 10."""
    s = 0.0
    flags = []
    passes = []
    disqualified = False
    
    # ==== HARD FILTERS (auto-skip if any fail) ====
    
    # Must have $5B+ market cap (liquidity)
    if d['market_cap'] < 5e9:
        flags.append('REJECT: <$5B mkt cap')
        disqualified = True
    
    # Must have analyst coverage
    if d.get('analyst_count', 0) < 3:
        flags.append('REJECT: No analyst coverage')
        disqualified = True
    
    # Must have earnings within 7 days (IV must be pumped)
    ...
    # Must have valid earnings stats
    if not es or not em or not p:
        ...
        # Edge ratio must be at least 1.5x
    ...
    if disqualified:
        return {'score': 0, 'flags': flags, 'passes': []}
    
    # ==== SOFT SCORING (out of 10) ====
    
    # 1. Edge ratio (max 4 points) — the most important factor
    ratio = em['expected_pct'] / es['avg_move']
    if ratio >= 4:
    ...
    # 2. Gap risk (max 2 points)
    rx = es['red_x_count']
    if rx == 0:
        s += 2; passes.append('Zero gaps 0/8')
    elif rx == 1:
        s += 1; passes.append('Low gaps 1/8')
    elif rx == 2:
        s += 0.5
        flags.append('Moderate gaps 2/8')
    
    # 3. Quality (max 2 points)
    rec = d.get('recommendation', '').lower()
    if rec == 'strong_buy':
        s += 2; passes.append('Strong Buy')
    elif rec in ('buy', 'moderate_buy'):
        s += 1; passes.append('Buy rating')
    elif rec == 'hold':
        s += 0.25
    
    # 4. Valuation (max 1 point)
    peg = d.get('peg')
    if peg and 0 < peg < 2:
    ...
    # 5. Trade structure (max 1 point)
    if p['pct_otm'] >= 35 and p['oi'] >= 100:
        s += 1; passes.append(f'{p["pct_otm"]:.0f}% OTM, deep OI')
    elif p['pct_otm'] >= 25 and p['oi'] >= 50:
        s += 0.5; passes.append('Safe strike')
    
    # 6. Earnings urgency bonus (max 0.5 — for fast IV crush)
    dte = d.get('days_to_earnings', 99)
    if dte <= 2:
        s += 0.5; passes.append(f'Fire in {dte}d')
    elif dte <= 4:
        s += 0.25
...
def render_html(results, scan_date):
    ...
    def build_row(r, rank=None):
        ...
        edge_color = '#16a34a' if r['edge_ratio'] >= 3 else '#ca8a04' if r['edge_ratio'] >= 1.5 else '#dc2626'
        score_color = '#16a34a' if r['score'] >= 7 else '#ca8a04' if r['score'] >= 5 else '#94a3b8'
...
<div class="container">
    ...
    <div class="stats">
        ...
        <div class="stat">
            ...
            <div class="stat-label">Top picks (≥7)</div>
        </div>
        <div class="stat">
            <div class="stat-num" style="color:#fbbf24">{len(watch)}</div>
            <div class="stat-label">Watch (5.0-6.9)</div>
        </div>
        <div class="stat">
            <div class="stat-num" style="color:#94a3b8">{len(skip)}</div>
            <div class="stat-label">Skip (&lt;5.0)</div>
        </div>
    </div>
    ...
    <div class="legend">
        <strong>Scoring (max 10):</strong> Edge ratio (4) · Gap risk (2) · Quality (2) · Valuation (1) · Strike (1) · Urgency bonus (0.5). <strong>Hard filters auto-reject:</strong> &lt;$5B mkt cap, no analyst coverage, earnings &gt;7d away, edge &lt;1.5x, ≥3 gaps in 8Q. ·
